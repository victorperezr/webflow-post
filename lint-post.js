...
async function getWebflowItems(limit, page) {
  const apiKey =
    "0ccad3ca2af58e662211e1ebe39bccc24a3db2a4bc6ce998430f8bc5c8b8352f";
    ...
    while (allItems.length < 1) {
      ...
      for (const item of items) {
        ...
        if (containsSpecialChars) {
          ...
          postBodyArray = allItems.map((item) =>
            item["post-body"].replace(/<[a-zA-Z\s"-=]+><\/[a-zA-Z\s"-=]+>/)
          );
          const postIdArray = allItems.map((item) => item._id);
          ...
          async function updateItems() {
            const url = `https://api.webflow.com/collections/64b0a27430b4c55a1642fd3b/items/${item._id}`;
            const options = {
              method: "PUT",
              headers: {
                accept: "application/json",
                "content-type": "application/json",
                Authorization: `Bearer 0ccad3ca2af58e662211e1ebe39bccc24a3db2a4bc6ce998430f8bc5c8b8352f`,
              },
              body: JSON.stringify({
                fields: { 
                    "_draft": false,
